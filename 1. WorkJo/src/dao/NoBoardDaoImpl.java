...
//		System.out.println("> > > anslectList 메소드 접속 < < <");
//		System.out.println("paging : "+paging);
		String sql = "";
		sql += "select * from("; 
		sql += " select rownum rnum, b.* from (";
		sql += "  select table_no, article_no, article_content, article_pw, article_title, revision_date, isdelete from tb_board b";
		sql += "   where isdelete=1";
		sql += "    and table_no = 20";
		sql += "   order by article_no desc";
		sql += "  )B";
		sql += " )tb_board";
...
//			System.out.println(" SQL 수행 ");
			...
			while(rs.next()) {
				Bbs b = new Bbs();
//				System.out.println(" > > > 쿼리 수행 결과 저장 < < <");
				//결과값 한 행 처리
				b.setTable_no(rs.getInt("table_no"));
				b.setArticle_no(rs.getInt("article_no"));
				b.setArticle_title(rs.getString("article_title"));
				b.setRevision_date(rs.getTimestamp("revision_date"));
				b.setArticle_content(rs.getString("article_content"));
