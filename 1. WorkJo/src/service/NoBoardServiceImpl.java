...
public class NoBoardServiceImpl implements NoBoardService {
	...
	@Override
	public List<Bbs> angetList(Paging paging) {
		System.out.println("> > > angetList 메소드 접속 < < <");
		return noBoardDao.anselectList(JDBCTemplate.getConnection(), paging);
	...
	@Override
	public Paging angetPaging(HttpServletRequest req) {
		//전달파라미터 curPage를 파싱한다
		String param = req.getParameter("curPage");
		int curPage = 0;
		if( param!=null && !"".equals(param) ) {
		...
		//Board 테이블의 총 게시글 수를 조회한다
		int totalCount = noBoardDao.anselectCntAll(JDBCTemplate.getConnection());
